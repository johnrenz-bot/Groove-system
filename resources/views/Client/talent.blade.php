@php
    ...
    /** @var \Illuminate\Support\Collection|\App\Models\Coach[] $coaches */
...
<a href="/client/home"
        ...
        <a href="{{ route('talent') }}"
           class="relative px-4 py-2 rounded-xl  border border-divider/50 shadow-inner  text-foreground/70 hover:text-foreground hover:bg-layer hover:border hover:border-divider/40 hover:shadow-md transition-all duration-300">
          Talents
        </a>
        <a href="{{ route('about') }}"
           class="relative   px-4 py-2 rounded-xl text-foreground/70 hover:text-foreground hover:bg-layer hover:border hover:border-divider/40 hover:shadow-md transition-all duration-300">
...
{{-- resources/views/Client/talent.blade.php --}}
  ...
  <section x-show="!selectedTalent" id="talent-section" class="flex items-center flex-col">
    ...
    <div class="flex justify-center flex-wrap gap-6">
      @foreach ([
        ...
        <div
          @click="onTalentChange('{{ $card['title'] }}'); viewMode='coach'; setQuery('view', viewMode);"
        ...
        <form id="coach-filters" method="GET" action="{{ url()->current() }}"
          ...
          <div
            x-data="{
              ...
              // Seed from server (keeps values on full reload)
  ...
  <section
    ...
    <div class="flex-1 space-y-8">
      <div class="flex items-center gap-5 px-6 py-5 rounded-2xl border-fg border backdrop-blur-md shadow-lg card">
        <div class="w-16 h-16 flex items-center justify-center rounded-full bg-card text-fg text-xs font-semibold uppercase border-4 border-divider shadow-md ring-2 ring-zinc-700 ring-offset-2 ring-fg">
      ...
      <div x-data="{ showForm: false }" class="card">
        <div
          @click="showForm = true"
          x-show="!showForm"
          class="flex items-center gap-3 px-4 py-3 border rounded-2xl backdrop-blur cursor-pointer transition hover:border-zinc-500"
        >
    ...
    <aside class="w-full lg:w-1/3 space-y-6">
      ...
      <div class="relative border border-divider/40 rounded-3xl p-6 shadow-sm bg-gradient-to-b from-card to-card/80 text-center hover:shadow-lg transition-all duration-300">
        ...
        <div class="mt-10">
          <h4 class="text-lg font-extrabold tracking-tight mb-1">Violation Notice</h4>
          <p class="text-sm text-foreground/60 mb-5">report any suspicious or harmful behavior</p>
          <button type="button" onclick="openTicketModal()" class="w-full py-3 rounded-xl bg-primary hover:bg-primary/90 active:scale-[0.98] text-fg font-semibold shadow-md transition-all duration-200">Report</button>
        </div>
      </div>
    </aside>
  </section>
    ...
    <div class="w-full max-w-xl bg-card text-foreground rounded-2xl shadow-2xl border border-divider/40 translate-y-6 opacity-0 transition-all duration-200" id="ticketPanel">
      <div class="flex items-center justify-between px-6 py-4 border-b border-divider/40">
        <div class="flex items-center gap-3">
          ...
          <h3 id="ticketTitle" class="text-lg font-semibold">Report</h3>
        </div>
  ...
  required
        ...
        <div class="pt-2">
          <button id="ticketSubmitBtn" type="submit"
            ...
            <span id="ticketBtnText">Submit </span>
          </button>
        </div>
      </form>
    </div>
