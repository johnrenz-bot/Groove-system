...
<body class="bg-slate-50 text-slate-800 min-h-screen flex flex-col relative">
  ...
  <script>
    ...
    const email = @json(session('clientEmail'));
    const emailSent = @json(session('emailSent'));
    if (!email || !emailSent) return;

    let tries = 0;
    const maxTries = 400; // ~20 minuto (400 * 3s)
    const int = setInterval(async () => {
      try {
        const res = await fetch(`{{ route('client.verification.status') }}?email=` + encodeURIComponent(email), {
        ...
        if (data?.verified) {
          clearInterval(int);
          // Friendly flag para makapagpakita ng success toast sa login
          window.location.href = `{{ route('login') }}?verified=1`;
        }
        if (++tries > maxTries) clearInterval(int);
      } catch (e) {
        // tahimik lang sa error; magpapatuloy ang polling
      }
    }, 3000);
  });
