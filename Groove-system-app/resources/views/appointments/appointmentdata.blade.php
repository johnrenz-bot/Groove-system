...
<div class="max-w-6xl mx-auto p-6 rounded-xl shadow-lg mt-20">
  <h1 class="text-3xl font-bold mb-6 text-center">Appointments List</h1>

  @if ($appointments->isEmpty())
    <p class="text-center text-zinc-400">No appointments found.</p>
  @else
    <div x-data="{ currentIndex: 0 }" class="space-y-6">
      @foreach ($appointments as $index => $appointment)
        <div x-show="currentIndex === {{ $index }}"
             ...
             class="max-w-4xl mx-auto bg-white/60 text-gray-900 rounded-lg shadow-md p-6 space-y-6">
          <div class="flex justify-between items-center border-b pb-4">
            <div>
              <h2 class="text-lg font-semibold">Request Appointment</h2>
              <span class="text-sm text-gray-500">Appointment ID #{{ $appointment->appointment_id }}</span>
            </div>

            <div class="flex flex-col items-center gap-3">
              <div class="flex gap-4">
                <button @click="currentIndex = (currentIndex - 1 + {{ $appointments->count() }}) % {{ $appointments->count() }}"
                        class="px-4 py-2 rounded-xl bg-white/10 text-white backdrop-blur-md border border-white/30 shadow hover:bg-white/20 transition">
                  <i class="fa-solid fa-chevron-left"></i>
                </button>
                <button @click="currentIndex = (currentIndex + 1) % {{ $appointments->count() }}"
                        class="px-4 py-2 rounded-xl bg-white/10 text-white backdrop-blur-md border border-white/30 shadow hover:bg-white/20 transition">
                  <i class="fa-solid fa-chevron-right"></i>
                </button>
              </div>
              <span class="text-sm text-gray-500">{{ $index + 1 }} of {{ $appointments->count() }}</span>
            </div>
          </div>

          <div>
            <h3 class="font-semibold text-gray-800">Client Information</h3>
            <div class="flex items-center space-x-4">
              ...
              @if (!empty($appointment->client?->photo))
                <img src="{{ asset('storage/' . $appointment->client->photo) }}" alt="Client Photo" class="w-20 h-20 rounded-full object-cover border border-zinc-700">
              @else
                <div class="w-20 h-20 rounded-full bg-gray-300 flex items-center justify-center text-gray-700 text-xl font-semibold select-none border border-zinc-400">
                  {{ $initials ?: 'NA' }}
                </div>
              @endif
              ...
              <div>
                ...
                <p class="text-sm text-gray-600">{{ $appointment->client->email ?? 'N/A' }}</p>
                <p class="text-sm text-gray-600">{{ $appointment->client->contact ?? 'N/A' }}</p>
              </div>
            </div>
          </div>
          ...
          <div class="bg-gray-100 p-4 rounded-md">
            ...
            <p class="text-gray-600">{{ $appointment->purpose ?? 'N/A' }}</p>
          </div>
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 bg-gray-50 p-4 rounded-md">
            <div>
              ...
              <p class="text-gray-700">{{ \Carbon\Carbon::parse($appointment->date)->format('l, d F Y') }}</p>
            </div>
            <div>
          ...
          <div>
            ...
            <p class="text-gray-600">{{ $appointment->experience ?? 'N/A' }}</p>
          </div>
    ...
    <i class="fa-solid fa-xmark mr-1"></i> Decline
          ...
          @if ($appointment->feedback)
            <div class="mt-4">
              ...
              <p class="text-gray-600 italic">"{{ $appointment->feedback }}"</p>
              <p class="text-sm text-gray-500">Rating: {{ $appointment->rating }}/5</p>
            </div>
          @endif
        </div>
      @endforeach
