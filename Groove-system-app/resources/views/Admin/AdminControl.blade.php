      ...
      <form id="themeForm" method="POST" action="{{ route('admin.updateTheme') }}" class="p-6">
        ...
        <div class="grid gap-5 sm:grid-cols-2 xl:grid-cols-3">
          @foreach (['light','dark','ocean'] as $theme)
            @php $is = $current === $theme; @endphp
            <label for="t-{{ $theme }}"
                   class="group block cursor-pointer rounded-xl border-2 p-4 transition
                          {{ $is ? 'border-[var(--color-primary)] shadow-md' : 'border-divider/40 hover:border-divider/60 hover:shadow-sm' }}">
              <div class="flex items-start gap-3">
                ...
                <div>
                  <div class="flex items-center gap-2">
                    ...
                    @if ($is)
                      <span class="rounded-full bg-layer px-2 py-0.5 text-[10px] font-semibold border border-divider/40" style="color: var(--color-primary)">Current</span>
                    @endif
                  </div>
